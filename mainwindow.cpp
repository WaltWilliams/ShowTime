...
void MainWindow::on_action_open_triggered() {
    home_path.clear();
    home_path = QFileDialog::getExistingDirectory(nullptr, ("Select A Directory"), QDir::homePath());

    if(home_path != "") {
        setWindowTitle(program_name + " " + QChar(0x2014) + " Current Directory: " + home_path);
    }
    else {
        setWindowTitle(program_name);
        file_list.clear();
    ...
    // which then loads the QLabel with a photo.
    get_the_filename = new GetFilename();
